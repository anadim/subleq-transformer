...
% ── Metadata ──
\title{\textbf{Learning to Execute a One-Instruction Computer:\\A Transformer Trained on SUBLEQ from Data Alone}}
\author{}
\date{}
...
\begin{abstract}
We train a standard transformer to execute SUBLEQ---a one-instruction, Turing-complete computer---from input--output examples alone, with no built-in knowledge of the instruction's semantics.
The model receives a machine state (program counter + memory) as input and predicts the state after one execution step.
At inference time, outputs are fed back as inputs, enabling the model to run arbitrary programs autoregressively.
On a 32-cell, 8-bit system (comparable in scale to the 1948 Manchester Baby), a 4.9M-parameter transformer achieves 100\% single-step accuracy on 2{,}000 held-out states and executes multi-step programs---including Fibonacci, integer multiplication, division, and square root---that were \emph{never} present in the training data.
We report scaling laws across model size and dataset size, identify that model \emph{width} (not depth) is the primary bottleneck for this task, and document the training recipe---curriculum learning, asymmetric loss weighting, and data regeneration---that makes convergence possible.
...
The question we ask is simple: \emph{can a neural network learn to be a SUBLEQ computer, purely from data?}
That is, given only examples of (state$_t$, state$_{t+1}$) pairs, can a transformer learn the semantics of subtract-and-branch well enough to execute \emph{arbitrary} programs at test time---including programs it has never seen during training?
...
A 4.9M-parameter Pre-LN transformer, trained for 80{,}000 steps on 100{,}000 regenerated examples per curriculum phase, reaches 100.0\% accuracy on held-out single-step evaluation and executes complex multi-step programs---Fibonacci, multiplication, division, and integer square root---with 100\% correctness, despite none of these programs appearing in the training set.

\begin{finding}[Main Result]
A standard encoder-only transformer with 4.9M parameters learns to execute SUBLEQ from data alone.
At test time, it functions as a general-purpose computer: it correctly runs programs (Fibonacci, multiplication, division, integer square root) that were \emph{never} seen during training, achieving 182/182 (100\%) on all tested emergent programs.
\end{finding}
...
We have demonstrated that a standard transformer can learn to execute SUBLEQ---a one-instruction, Turing-complete computer---from data alone.
The key ingredients are: (1)~problem miniaturization to a tractable scale, (2)~asymmetric loss weighting to overcome the copy-computation imbalance, (3)~curriculum learning to build from simple to complex programs, and (4)~sufficient model width to provide information routing bandwidth.

The resulting model functions as a learned general-purpose computer: given any valid SUBLEQ state as input, it correctly predicts the next state, and by autoregressive application, it can execute arbitrary programs---including ones never seen during training.
On all 182 tested instances of emergent programs (multiplication, Fibonacci, division, square root), the wide model achieves 100\% accuracy.

This establishes that transformers are capable of learning complete instruction-set semantics from examples, bridging the gap between pattern recognition and algorithmic execution.

\begin{finding}[Summary of Key Findings]
\begin{enumerate}[nosep,leftmargin=1.2em]
  \item A 4.9M-parameter transformer learns SUBLEQ execution from data alone, achieving 100\% single-step eval accuracy.
  \item The model generalizes to execute programs never seen during training (Fibonacci, multiplication, division, square root) with 100\% correctness.
